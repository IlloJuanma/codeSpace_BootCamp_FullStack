...
function getBestMovies(arrayMovies){
    // Calcular valoración
    // Volvemos a usar reduce, donde el acumulador es una variable con el valor acumulado en cada iteración
    // movie.rating es el rating de la pelicula actual en esa iteración
    // 0 es el valor inicial, que necesitamos apra ir sumando (en el otro ejercicio usamos un objeto como valor inicial {})

    const bestRating = arrayMovies.reduce((acc,movie) => acc + movie.rating, 0) / arrayMovies.length;

    // Filtramos por valoración

    return arrayMovies
        .filter(movie => movie.rating > bestRating)
